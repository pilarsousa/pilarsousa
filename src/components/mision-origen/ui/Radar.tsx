...
/**
 * Animated radar sweep rendered on a WebGL canvas — used as the backdrop of
 * the closing CTA card.
 *
 * Adapted from the React Bits "Radar" component: ported to TypeScript, marked
 * as a client component (it touches the DOM and WebGL), and given a
 * reduced-motion guard. The original's companion CSS file is folded into the
 * inline style below, since it was only setting width/height to 100%.
 *
 * It is decorative: the canvas is aria-hidden and non-interactive, and the
 * caller is responsible for layering readable content above it.
 */
...
type RadarProps = {
  ...
  sweepLobes?: number;
  /** Primary radar colour, hex. */
  color?: string;
  /** Background colour, hex. */
  backgroundColor?: string;
  /** Edge fade intensity based on distance from centre. */
  falloff?: number;
  /** Overall brightness multiplier. */
  brightness?: number;
  /** Enable cursor-reactive centre offset. */
...
export function Radar({
  ...
  useEffect(() => {
    ...
    /* Decorative animation: skip the WebGL context entirely for visitors who
       ask for reduced motion, rather than rendering a frozen frame. */
    ...
    /* The card resizes with its content and on breakpoint changes, not just
       with the window — observe the element itself, not only window.resize. */
